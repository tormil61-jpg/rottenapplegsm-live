exports.handler = async (event) => {
  ...
  try {
    ...
    const { amount, currency = 'USD', description, orderId, customerEmail, metadata = {} } = body;
    if (!amount || isNaN(parseFloat(amount))) return { statusCode: 400, body: JSON.stringify({ error: 'Invalid amount' }) };
    const p = new URLSearchParams({
      'payment_method_types[]': 'card',
      'line_items[0][price_data][currency]': currency.toLowerCase(),
      'line_items[0][price_data][product_data][name]': description || 'Rotten Apple GSM Service',
      'line_items[0][price_data][unit_amount]': Math.round(parseFloat(amount)*100).toString(),
      'line_items[0][quantity]': '1',
      'mode': 'payment',
      'success_url': 'https://www.rottenapplegsm.com/?stripe=success&order='+(orderId||''),
      'cancel_url': 'https://www.rottenapplegsm.com/?stripe=cancel',
      'metadata[orderId]': orderId||''
    });
    // Add all metadata
    Object.entries(metadata).forEach(([k,v]) => {
      if(k !== 'orderId') p.append('metadata['+k+']', String(v||'').slice(0,500));
    });
    ...
    if (!res.ok) return { statusCode: res.status, headers: {'Access-Control-Allow-Origin':'*'}, body: JSON.stringify({ error: s.error?.message || 'Stripe error' }) };
